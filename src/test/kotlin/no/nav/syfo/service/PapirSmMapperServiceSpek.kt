...
class PapirSmMapperServiceSpek :
    FunSpec({
        ...
        context("PapirSmMappingService") {
            test("test OCR file with empty behandlngsdager") {
                val ocrFil =
                    skanningMetadataUnmarshaller.unmarshal(
                        StringReader(
                            getFileAsString(
                                "src/test/resources/ocr-sykmelding-komplett-uten-behandlingsdager.xml"
                            )
                        )
                    ) as Skanningmetadata

                val papirSm =
                    mapOcrFilTilPapirSmRegistrering(
            ...
            test("Test ocr file with empty hoveddiagnoselist") {
                val ocrFil =
                    skanningMetadataUnmarshaller.unmarshal(
                        StringReader(
                            getFileAsString(
                                "src/test/resources/ocr-sykmelding-komplett-uten-hoveddiagnose.xml"
                            )
                        )
                    ) as Skanningmetadata
            ...
            test("Tests a complete OCR file, and should parse fine") {
                ...
                papirSm.syketilfelleStartDato shouldBeEqualTo
                    ocrFil.sykemeldinger.syketilfelleStartDato

                papirSm.arbeidsgiver?.harArbeidsgiver shouldBeEqualTo
                    HarArbeidsgiver.EN_ARBEIDSGIVER
                papirSm.arbeidsgiver?.navn shouldBeEqualTo
                    ocrFil.sykemeldinger.arbeidsgiver.navnArbeidsgiver
                papirSm.arbeidsgiver?.stillingsprosent shouldBeEqualTo
                    ocrFil.sykemeldinger.arbeidsgiver.stillingsprosent.toInt()
                papirSm.arbeidsgiver?.yrkesbetegnelse shouldBeEqualTo
                    ocrFil.sykemeldinger.arbeidsgiver.yrkesbetegnelse
                ...
                papirSm.medisinskVurdering?.yrkesskade shouldBeEqualTo
                    ocrFil.sykemeldinger.medisinskVurdering.isYrkesskade

                papirSm.skjermesForPasient shouldBeEqualTo
                    ocrFil.sykemeldinger.medisinskVurdering.isSkjermesForPasient
                ...
                papirSm.prognose?.hensynArbeidsplassen shouldBeEqualTo
                    "Dette vet jeg ikke hva betyr engang."
                ...
                papirSm.prognose?.erIkkeIArbeid?.vurderingsdato shouldBeEqualTo
                ...
                papirSm.utdypendeOpplysninger?.get("6.2")?.get("6.2.4")?.sporsmal shouldBeEqualTo
                    "Beskriv pågående og planlagt henvisning,utredning og/eller behandling"
                papirSm.utdypendeOpplysninger?.get("6.2")?.get("6.2.4")?.svar shouldBeEqualTo
                    "Rolig ferie i kullfritt miljø"
                papirSm.utdypendeOpplysninger
                    ?.get("6.2")
                    ?.get("6.2.4")
                    ?.restriksjoner
                    ?.first() shouldBeEqualTo SvarRestriksjon.SKJERMET_FOR_ARBEIDSGIVER
                ...
                papirSm.meldingTilNAV?.beskrivBistand shouldBeEqualTo
                    "Her må NAV få fingeren ut. Skandale!"
                ...
                papirSm.kontaktMedPasient?.begrunnelseIkkeKontakt shouldBeEqualTo
                    "Han var syk i 2000 også."
