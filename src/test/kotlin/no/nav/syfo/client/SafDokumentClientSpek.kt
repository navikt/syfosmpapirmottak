...
class SafDokumentClientSpek :
    FunSpec({
        ...
        val mockServer =
            embeddedServer(Netty, mockHttpServerPort) {
                    ...
                    routing {
                        ...
                        get(
                            "/saf/rest/hentdokument/journalpostId/dokumentInfoIdUgyldigDok/ORIGINAL"
                        ) {
        ...
        val safDokumentClient =
            SafDokumentClient("$mockHttpServerUrl/saf", accessTokenClientV2, "scope", httpClient)
        ...
        context("SafDokumentClient håndterer respons korrekt") {
            test("Mapper mottatt dokument korrekt") {
                ...
                skanningmetadata
                    ...
                    ?.hovedDiagnose
                    ?.first()
                    ?.diagnosekode shouldBeEqualTo "900.3"
                skanningmetadata
                    ?.sykemeldinger
                    ?.medisinskVurdering
                    ?.hovedDiagnose
                    ?.first()
                    ?.diagnose shouldBeEqualTo "Skikkelig syk"
                skanningmetadata
                    ?.sykemeldinger
                    ?.aktivitet
                    ?.aktivitetIkkeMulig
                    ?.periodeFOMDato shouldBeEqualTo LocalDate.of(2019, Month.JANUARY, 10)
                skanningmetadata
                    ?.sykemeldinger
                    ?.aktivitet
                    ?.aktivitetIkkeMulig
                    ?.periodeTOMDato shouldBeEqualTo LocalDate.of(2019, Month.JANUARY, 14)
                skanningmetadata
                    ?.sykemeldinger
                    ?.aktivitet
                    ?.aktivitetIkkeMulig
                    ?.medisinskeArsaker
                    ?.isMedArsakerHindrer shouldBeEqualTo true
                skanningmetadata?.sykemeldinger?.tilbakedatering?.tilbakebegrunnelse shouldBeEqualTo
                    "Legevakten\n" + "                Sentrum"
